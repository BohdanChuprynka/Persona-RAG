...
async def update_memory_node(state: GraphState) -> GraphState:
    """Distill the running session into a long-form contact memory summary.

    Throttled: only fires every ``MEMORY_UPDATE_INTERVAL_TURNS`` completed
    turns to avoid doubling OpenAI cost + latency on every message.
    """
    ...
    await update_contact_memory(user_id=state["user_id"], session=session)
    return state
