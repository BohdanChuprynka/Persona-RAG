...
CELLS = [
    ...
    code(
        "# NO %%capture — you want to SEE the resolver. Two things are needed to move",
        "# transformers off Colab's stale 4.46.3 (which fails with 'PreTrainedConfig'):",
        "#   PIP_CONSTRAINT= : Colab caps libs via a constraints file; this lifts it.",
        "#   --upgrade-strategy eager : a plain --upgrade leaves a 'good enough' transformers.",
    ...
    md(
        "## 5b. Mount Google Drive — so checkpoints survive a disconnect",
        "Checkpoints go to **Drive**, not `/content` (which is wiped the moment the T4 drops).",
        "If your session dies or you hit the free-GPU limit: reconnect later, **Run all** again",
        "(install → restart → reload model, ~6 min), and the Train cell **auto-resumes from the",
        "last Drive checkpoint** instead of starting from zero.",
    ),
    code(
    ...
    md(
        ...
        "single most important switch for style mimicry. `bs=4 × ga=4` = effective batch 16.",
        "Checkpoints are written to Drive every `save_steps`, so a dropped session costs at most",
        "~25 min and the run resumes from the last one.",
    ),
    code(
        ...
        "        warmup_ratio = 0.05,",
        "        num_train_epochs = 1,              # 1 epoch on the ~10k recency set learns the voice; bump to 2 (resume makes it cheap) only if VOICE_DISTANCE wants more",
        "        learning_rate = 2e-4,",
        "        logging_steps = 20,",
        '        optim = "adamw_8bit",',
        "        weight_decay = 0.01,",
        ...
        "        seed = 3407,",
        '        save_strategy = "steps",',
        "        save_steps = 200,                  # ~25 min between saves at T4 speed; lower = safer",
        "        save_total_limit = 3,",
        "        output_dir = OUTPUT_DIR,           # Drive path from cell 5b — survives a disconnect",
        '        report_to = "none",',
        "        # New TRL defaults padding_free=True, which refuses to enforce max_length",
        "        # without packing. We need packing=False for train_on_responses_only's",
        "        # per-example masking, so disable padding_free -> classic padded+truncated.",
        "        padding_free = False,",
        "    ),",
        ")",
        "",
    ...
    md(
        "## 7. Train (auto-resumes from the last Drive checkpoint)",
        "First run starts fresh; after a disconnect + Run-all it picks up where it left off —",
        "watch the step counter continue from the last saved step, not 0.",
    ),
    code(
