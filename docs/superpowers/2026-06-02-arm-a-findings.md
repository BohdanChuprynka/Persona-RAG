...
Per the decide rule (spec §2: a voice-tie + cheaper/local ships the LoRA), the automatic-metric verdict is **the LoRA ships** — its edge narrows under the API's machinery but does not disappear, and cost/privacy break the ties. This is **weakly corroborated** by an informal self-read (Bohdan found the bare-API replies obviously off-voice) — but that read is **recall-biased** (he recognizes his own messages), unscored, and with private content barring outside raters, so the pre-registered **primary human win-rate is UNRESOLVED, not a third independent method**. The metric verdict (arms B + A) stands on its own. The open frontier is the *Turing* test — is the LoRA distinguishable from Bohdan's **real** replies? — though the same recall bias means he cannot run it on himself unbiased (see `2026-06-02-turing-test-design.md`).
